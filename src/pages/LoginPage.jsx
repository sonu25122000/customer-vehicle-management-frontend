...
function EyeIcon({ off, ...props }) {
  return off ? (
    ...
    >
      <path
        d="M2 12c1.6-3.7 5.4-7 10-7s8.4 3.3 10 7c-1.6 3.7-5.4 7-10 7s-8.4-3.3-10-7Z"
        strokeLinejoin="round"
      />
...
function SignInIcon(props) {
  return (
    ...
    >
      ...
      <path
        d="M11 12h10m0 0-3.5-3.5M21 12l-3.5 3.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        ...
        <div className="hidden flex-col justify-between p-10 md:flex md:pl-14 md:pt-8 lg:pl-20">
          ...
          <div>
            ...
            <p className="mt-4 max-w-sm text-sm text-slate-600">
              Manage customers, vehicles, bookings, tolls and payments — all in
              one secure workspace built for speed, accuracy and control.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {FEATURES.map(({ icon: Icon, title, sub }) => (
              <div
                key={title}
                className="rounded-xl bg-white/90 p-3 text-center shadow-sm ring-1 ring-black/5 backdrop-blur-sm"
              >
                <Icon className="mx-auto h-5 w-5 text-blue-600" />
                <p className="mt-1.5 text-xs font-bold text-slate-800">
                  {title}
                </p>
                <p className="text-[0.65rem] leading-tight text-slate-500">
                  {sub}
                </p>
              </div>
            ))}
        ...
        <div className="mx-auto my-6 flex w-[calc(100%-2rem)] max-w-md flex-col justify-between self-start rounded-3xl bg-[#0d1a3f] p-6 shadow-2xl sm:p-10 md:mx-0 md:ml-auto md:mr-8 md:mt-8">
          ...
          <div className="border-t border-white/10 pt-4 text-center">
            <p className="flex items-center justify-center gap-1.5 text-xs font-semibold text-blue-100">
              <ShieldCheckIcon className="h-3.5 w-3.5" /> Secure &middot;
